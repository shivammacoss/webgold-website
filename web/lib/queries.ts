...
import { api } from "./api";
import type {
  FD, FDPlan, GoldRate, GoldRatePoint, Portfolio,
  ReferralSummary, Transaction, User, Wallet,
} from "./types";

export const qk = {
  me: ["me"] as const,
  rate: ["gold", "rate"] as const,
  history: (range: string) => ["gold", "history", range] as const,
...
export const useRateHistory = (range: "1d" | "1w" | "1m" | "1y" = "1w") =>
  useQuery({
    queryKey: qk.history(range),
    queryFn: () => api<GoldRatePoint[]>(`/gold/rate/history?range=${range}`, { auth: false }),
  });
...
export const useSellGold = () => {
  ...
  return useMutation({
    mutationFn: (grams: number) =>
      api<Wallet>("/invest/gold/sell", { method: "POST", body: JSON.stringify({ grams }) }),
    onSuccess: inv,
  });
};
