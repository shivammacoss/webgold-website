...
/** Rough static fallback used until /gold/rate has loaded. INR per 1 unit. */
...
export function useCurrency() {
  ...
  // INR-per-1-unit for the active currency. Comes from /gold/rate.fx when
  // available, else from the static fallback.
  const inrPer = rate?.fx?.[currency] ?? FX_FALLBACK[currency];

  /** Format an INR amount as the user's display currency. */
  const formatMoney = (
    inrAmount: number,
  ...
  /** Convert an INR amount to the active currency (raw number). */
  const convert = (inr: number) => inr / inrPer;

  /** Convert a display-currency amount back to INR (for form submissions). */
